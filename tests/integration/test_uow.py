...
def insert_user_and_match(session):
    session.execute(
        text("INSERT INTO matches (home_team_id, away_team_id) VALUES (1, 2)")
    )
    session.execute(
        text("INSERT INTO users (username) VALUES (:username)"), dict(username="Bob")
    )


def test_uow_can_retrieve_match_and_place_a_bet_to_it(
    in_memory_sqlite_session_factory: sessionmaker,
):
    user_id = 1
    match_id = 1
    ...
    with uow:
        ...
        bet = schema.Bet(
            user_id=user_id, match_id=match.id, home_team_score=2, away_team_score=3
        )
        match.place_bet(bet)
